...
void CFilteringView::AverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			col = m_mfcImage1->GetPixel(c,r);
			imageBits[r*m_width+c] = GetRValue(col); //f 
	...
	for (r=0; r<m_height; r++) { // n = r = height, m = c = width
		for (c=0; c<m_width; c++) {
			mean = 0;
			//
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			else 
			{
					for (k = -w ; k <= w ; k++)
					{
						if( (r + k >= 0) && (c - w - 1 >= 0) && (r + k < m_height) && (c - w -1  < m_width))
							mean -= m_imageBits[(r + k) * m_width + (c - w - 1 )] * ((double) 1/((double)w0*w0));
					}

					for (k = -w ; k <= w ; k++)
					{
						if( (r + k >= 0) && (c + w >= 0) && (r + k < m_height) && (c + w  < m_width))
							mean += m_imageBits[(r + k) * m_width + (c + w)] * ((double) 1/((double)w0*w0));
					}
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for(l = -w ; l < w ; l++)
			{
				...
				{
					if((r + l >= 0) && (c + k >= 0) && (r + l < m_height) && (c + k  < m_width))
						medarray[(w+l)*w0 + (w+k)] = m_imageBits[(r + l )*m_width + (c + k)];
					else
						medarray[(w+l)*w0 + (w+k)] = 0;
				}
			}
			QuickMedian(medarray,0,K,K/2);
