...
void CPCAfaceRecognitionView::OnFileOpen()
{
	...
	*****/
	...
	Mat Proj_q = V_reduced.t()*U_q;  //테스트
	...
	for (i=0; i<Proj_q.cols; i++) {
		...
		//
		Mat k1(Proj_q,Rect(i,0,1,Proj_q.rows));
		for(j = 0 ; j < Proj_p.cols; j++)
		{
			sum = 0;
			Mat k2(Proj_p,Rect(j,0,1,Proj_p.rows));
			Mat temp = k1 - k2;
			temp = temp.mul(temp); 
			for (k=0; k<temp.rows; k++)
			sum += temp.at<float>(k, 0);
			dist.at<float>(i, j) = sqrt(sum);
		}
